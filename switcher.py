...
async def save_image(url, img_path):
    ...
    """
    if launch is None:
        raise RuntimeError('未安装pyppeteer，无法渲染开关状态图片')
    ...
    for group in g_list:
        ...
        if not switcher_dict.get(gid):
            ...
            for func in admin_funcs:
                if func in ['img_check', 'auto_ban', 'group_msg', 'particular_e_notice', 'group_recall', 'cleanup']:
...
async def switcher_handle(gid, matcher, user_input_func_name):
    user_input_func_name = str(user_input_func_name).strip()
    if not user_input_func_name:
        await fi(matcher, '请输入要开关的功能名')
        return
    for func in admin_funcs:
