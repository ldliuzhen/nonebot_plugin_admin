...
from .admin_role import DEPUTY_BAN, DEPUTY_KICK, DEPUTY_BLACK, DEPUTY_CHANGE, DEPUTY_RECALL, DEPUTY_ESSENCE
...
DEPUTY_BAN_TIME_LIMIT = 2 * 60 * 60


def _is_limited_deputy(event: GroupMessageEvent) -> bool:
    if event.user_id in su or str(event.user_id) in su:
        return False
    sender = getattr(event, "sender", None)
    if getattr(sender, "role", None) in {"owner", "admin"}:
        return False
    admins = g_admin()
    return event.user_id in admins.get(str(event.group_id), [])

ban = on_command('禁', priority=2, block=True, permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER | DEPUTY_BAN)
@ban.handle()
async def _(bot: Bot, matcher: Matcher, event: GroupMessageEvent, msg: str = Depends(msg_text),
    ...
    try:
        ...
        time = None
    if sb:
        deputy_max_time = DEPUTY_BAN_TIME_LIMIT if _is_limited_deputy(event) else None
        time_limited = time is not None and deputy_max_time is not None and time > deputy_max_time
        if time_limited:
            time = deputy_max_time
        baning = mute_sb(bot, event.group_id, lst=sb, time=time, max_time=deputy_max_time)
        try:
            async for baned in baning:
                if baned:
                    await baned
            if time_limited:
                await log_fi(matcher, '分管禁言时间超过2小时，已按2小时执行')
            else:
                await log_fi(matcher, '禁言操作成功' if time is not None else '用户已被禁言随机时长')
        except ActionFailed:
            await fi(matcher, '权限不足')

unban = on_command('解', priority=2, block=True, permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER | DEPUTY_BAN)
@unban.handle()
...
ban_all = on_command('/all', priority=2, aliases={'/全员'}, block=True,
                     permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER | DEPUTY_BAN)
@ban_all.handle()
async def _(bot: Bot, matcher: Matcher, event: GroupMessageEvent, msg: str = Depends(msg_text)):
    """
...
change = on_command('改', priority=2, block=True, permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER | DEPUTY_CHANGE)
@change.handle()
async def _(bot: Bot, matcher: Matcher, event: GroupMessageEvent, msg: str = Depends(msg_text),
            sb: list = Depends(msg_at)):
    """
...
kick = on_command('踢', priority=2, block=True, permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER | DEPUTY_KICK)
@kick.handle()
async def _(bot: Bot, matcher: Matcher, event: GroupMessageEvent, sb: list = Depends(msg_at)):
    """
    /踢 @user 踢出某人
...
kick_ = on_command('黑', priority=2, block=True, permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER | DEPUTY_BLACK)
@kick_.handle()
async def _(bot: Bot, matcher: Matcher, event: GroupMessageEvent, sb: list = Depends(msg_at)):
    """
    黑 @user 踢出并拉黑某人
...
set_essence = on_command("加精", priority=2, aliases={'加精', 'set_essence'}, block=True,
                         permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER | DEPUTY_ESSENCE)
@set_essence.handle()
...
msg_recall = on_command('撤回', priority=2, aliases={'recall'}, block=True,
                        permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER | DEPUTY_RECALL)
@msg_recall.handle()
async def _(bot: Bot, matcher: Matcher, event: GroupMessageEvent, msg: str = Depends(msg_text),
