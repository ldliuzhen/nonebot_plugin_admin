# python3
# -*- coding: utf-8 -*-
# @Time    : 2022/1/16 22:21
# @Author  : yzyyz
# @Email   :  youzyyz1384@qq.com
# @File    : word_analyze.py
# @Software: PyCharm
import datetime
import json
import os

try:
    import httpx
except ModuleNotFoundError:
    from . import compat_httpx as httpx
from nonebot import on_command, logger, on_message
from nonebot.adapters import Message
from nonebot.adapters.onebot.v11.permission import GROUP_ADMIN, GROUP_OWNER
from nonebot.matcher import Matcher
from nonebot.params import CommandArg
from nonebot.params import Depends
from nonebot.permission import SUPERUSER

from .message import *
from .path import *
from .utils import del_txt_line, add_txt_line, get_txt_line, json_upload, json_load, get_group_path, resolve_group_alias

try:
    import jieba
    _HAS_JIEBA = True
except ImportError:
    _HAS_JIEBA = False

# ============ 词频内存缓存 ============
_freq_cache: dict[str, dict[str, int]] = {}
_freq_dirty: dict[str, int] = {}
_FLUSH_EVERY = 50


def _load_freq(gid: str) -> dict[str, int]:
    if gid not in _freq_cache:
        path = words_contents_path / f"{gid}.json"
        if path.exists():
            try:
                with open(path, 'r', encoding='utf-8') as f:
                    _freq_cache[gid] = json.load(f)
            except Exception:
                _freq_cache[gid] = {}
        else:
            _freq_cache[gid] = {}
    return _freq_cache[gid]


def flush_freq(gid: str):
    if gid in _freq_cache and _freq_dirty.get(gid, 0) > 0:
        path = words_contents_path / f"{gid}.json"
        with open(path, 'w', encoding='utf-8') as f:
            json.dump(_freq_cache[gid], f, ensure_ascii=False, separators=(',', ':'))
        _freq_dirty[gid] = 0

word_start = on_command('记录本群', priority=2, block=True, permission=GROUP_ADMIN | GROUP_OWNER | SUPERUSER)


def _load_counter(path: Path) -> dict:
    return json_load(path) or {}


async def _resolve_cross_group(bot: Bot, event: GroupMessageEvent, args: Message):
    arg_text = str(args).strip()
    if not arg_text:
        return event.group_id, ''
    if arg_text.isdigit():
        target_gid = int(arg_text)
    else:
        resolved = resolve_group_alias(arg_text)
        if not resolved:
            return event.group_id, ''
        target_gid = int(resolved)
    info = await bot.get_group_info(group_id=target_gid)
    name = info.get('group_name', str(target_gid))
    return target_gid, f"【{name}】\n"


@word_start.handle()
async def _(matcher: Matcher, event: GroupMessageEvent):
    gid = str(event.group_id)
    with open(word_path, 'r+', encoding='utf-8') as c:
        txt = c.read().split('\n')
        if gid not in txt:
            c.write(gid + '\n')
            logger.info(f"开始记录{gid}")
            await matcher.finish('成功')
        logger.info(f"{gid}已存在")
        await matcher.finish(f"{gid}已存在")


word_stop = on_command('停止记录本群', priority=2, block=True, permission=GROUP_ADMIN | GROUP_OWNER | SUPERUSER)


@word_stop.handle()
async def _(matcher: Matcher, event: GroupMessageEvent):
    gid = str(event.group_id)
    groups = [line for line in word_path.read_text(encoding='utf-8').splitlines() if line]
    if gid in groups:
        groups = [line for line in groups if line != gid]
        with open(word_path, 'w', encoding='utf-8') as c:
            c.write('\n'.join(groups) + ('\n' if groups else ''))
            logger.info(f"停止记录{gid}")
            await matcher.finish('成功，曾经的记录不会被删除')
    else:
        logger.info(f"停用失败：{gid}不存在")
        await matcher.finish(f"停用失败：{gid}不存在")


word = on_message(priority=3, block=False)


@word.handle()
async def _(event: GroupMessageEvent, msg: str = Depends(msg_text_no_url)):
    """
    记录聊天内容
    :param event:
    :return:
    """
    gid = str(event.group_id)
    uid = str(event.user_id)
    message_path_group = group_message_data_path / f"{gid}"
    # datetime获取今日日期
    today = datetime.datetime.now().strftime('%Y-%m-%d')
    if not os.path.exists(message_path_group):
        os.mkdir(message_path_group)
    if not os.path.exists(message_path_group / f"{today}.json"):  # 日消息条数记录 {uid：消息数}
        json_upload(message_path_group / f"{today}.json", {uid: 1})
    else:
        dic_ = json_load(message_path_group / f"{today}.json")
        if uid not in dic_:
            dic_[uid] = 1
        else:
            dic_[uid] += 1
        json_upload(message_path_group / f"{today}.json", dic_)
    if not os.path.exists(message_path_group / 'history.json'):  # 历史发言条数记录 {uid：消息数}
        json_upload(message_path_group / 'history.json', {uid: 1})
    else:
        dic_ = json_load(message_path_group / 'history.json')
        if uid not in dic_:
            dic_[uid] = 1
        else:
            dic_[uid] += 1
        json_upload(message_path_group / 'history.json', dic_)
    txt = word_path.read_text(encoding='utf-8').split('\n')
    if gid in txt:
        from .wordcloud import is_wc_blocked
        if is_wc_blocked(gid, uid):
            return
        if _HAS_JIEBA:
            words = jieba.lcut(msg)
        else:
            words = msg.split()
        freq = _load_freq(gid)
        changed = False
        for w in words:
            w = w.strip()
            if len(w) <= 1:
                continue
            freq[w] = freq.get(w, 0) + 1
            changed = True
        if changed:
            _freq_dirty[gid] = _freq_dirty.get(gid, 0) + 1
            if _freq_dirty[gid] >= _FLUSH_EVERY:
                flush_freq(gid)

stop_words_add = on_command('添加停用词', priority=2, aliases={'增加停用词', '新增停用词'}, block=True,
                            permission=GROUP_ADMIN | GROUP_OWNER | SUPERUSER)


@stop_words_add.handle()
async def _(event: GroupMessageEvent, matcher: Matcher, args: Message = CommandArg()):
    """
    添加停用词
    """
    await add_txt_line(get_group_path(event, stop_words_path), matcher, args, '停用词')


stop_words_del = on_command('删除停用词', priority=2, aliases={'移除停用词', '去除停用词'}, block=True,
                            permission=GROUP_ADMIN | GROUP_OWNER | SUPERUSER)


@stop_words_del.handle()
async def _(event: GroupMessageEvent, matcher: Matcher, args: Message = CommandArg()):
    """
    删除停用词
    """
    await del_txt_line(get_group_path(event, stop_words_path), matcher, args, '停用词')


stop_words_list = on_command('停用词列表', priority=2, aliases={'查看停用词', '查询停用词'}, block=True,
                             permission=GROUP_ADMIN | GROUP_OWNER | SUPERUSER)


@stop_words_list.handle()
async def _(event: GroupMessageEvent, matcher: Matcher):
    """
    停用词列表
    """
    await get_txt_line(get_group_path(event, stop_words_path), matcher, '停用词')


update_mask = on_command('更新mask', priority=2, aliases={'下载mask'}, block=True,
                         permission=GROUP_ADMIN | GROUP_OWNER | SUPERUSER)


@update_mask.handle()
async def _(matcher: Matcher):
    """
    更新mask
    """
    already_have = len(os.listdir(wordcloud_bg_path))
    try:
        async with httpx.AsyncClient() as client:
            num_in_cloud = int((await client.get(
                'https://fastly.jsdelivr.net/gh/yzyyz1387/blogimages/nonebot/wordcloud/num.txt')).read())
            if num_in_cloud > already_have:
                await matcher.send('正zhai更新中...')
                for i in range(already_have, num_in_cloud):
                    img_content = (await client.get(
                        f"https://fastly.jsdelivr.net/gh/yzyyz1387/blogimages/nonebot/wordcloud/bg{i}.png")).content
                    with open(wordcloud_bg_path / f"{i}.png", 'wb') as f:
                        f.write(img_content)
                await matcher.send('更新完成（好耶）')
            elif num_in_cloud == already_have:
                await matcher.send('蚌！已经是最新了耶')
    except Exception as e:
        logger.info(e)
        await matcher.send(f"QAQ,更新mask失败:\n{e}")


# FIXME: 这一块重复代码有点多了
who_speak_most_today = on_command('今日榜首', priority=2, aliases={'今天谁话多', '今儿谁话多', '今天谁屁话最多'},
                                  block=True)


@who_speak_most_today.handle()
async def _(bot: Bot, matcher: Matcher, event: GroupMessageEvent, args: Message = CommandArg()):
    try:
        gid, header = await _resolve_cross_group(bot, event, args)
    except Exception:
        await matcher.finish('无法查询该群，可能机器人不在该群中')
    today = datetime.date.today().strftime('%Y-%m-%d')
    dic_ = _load_counter(group_message_data_path / f"{gid}" / f"{today}.json")
    if not dic_:
        await matcher.finish(header + '没有任何人说话')
    top = sorted(dic_.items(), key=lambda x: x[1], reverse=True)
    top = (await member_in_group(bot, gid, top))
    if len(top) == 0:
        await matcher.finish(header + '没有任何人说话')
    nickname = await safe_get_nickname(bot, gid, int(top[0][0]))
    await matcher.finish(header + f"太强了！今日榜首：\n{nickname}，发了{top[0][1]}条消息")


speak_top = on_command('今日发言排行', priority=2, aliases={'今日排行榜', '今日发言排行榜', '今日排行'}, block=True)


@speak_top.handle()
async def _(bot: Bot, matcher: Matcher, event: GroupMessageEvent, args: Message = CommandArg()):
    try:
        gid, header = await _resolve_cross_group(bot, event, args)
    except Exception:
        await matcher.finish('无法查询该群，可能机器人不在该群中')
    today = datetime.date.today().strftime('%Y-%m-%d')
    dic_ = _load_counter(group_message_data_path / f"{gid}" / f"{today}.json")
    if not dic_:
        await matcher.finish(header + '没有任何人说话')
    top = sorted(dic_.items(), key=lambda x: x[1], reverse=True)
    top = (await member_in_group(bot, gid, top))
    if len(top) == 0:
        await matcher.finish(header + '没有任何人说话')
    top_list = []
    for i in range(min(len(top), 10)):
        nickname = await safe_get_nickname(bot, gid, int(top[i][0]))
        top_list.append(f"{i + 1}. {nickname}，发了{top[i][1]}条消息")
    await matcher.finish(header + '\n'.join(top_list))


speak_top_yesterday = on_command('昨日发言排行', priority=2, aliases={'昨日排行榜', '昨日发言排行榜', '昨日排行'},
                                 block=True)


@speak_top_yesterday.handle()
async def _(bot: Bot, matcher: Matcher, event: GroupMessageEvent, args: Message = CommandArg()):
    try:
        gid, header = await _resolve_cross_group(bot, event, args)
    except Exception:
        await matcher.finish('无法查询该群，可能机器人不在该群中')
    yesterday = (datetime.date.today() - datetime.timedelta(days=1)).strftime('%Y-%m-%d')
    if os.path.exists(group_message_data_path / f"{gid}" / f"{yesterday}.json"):
        dic_ = _load_counter(group_message_data_path / f"{gid}" / f"{yesterday}.json")
        if not dic_:
            await matcher.finish(header + '没有任何人说话')
        top = sorted(dic_.items(), key=lambda x: x[1], reverse=True)
        top = (await member_in_group(bot, gid, top))

        if len(top) == 0:
            await matcher.finish(header + '没有任何人说话')
        top_list = []
        for i in range(min(len(top), 10)):
            nickname = await safe_get_nickname(bot, gid, int(top[i][0]))
            top_list.append(f"{i + 1}. {nickname}，发了{top[i][1]}条消息")

        await matcher.finish(header + '\n'.join(top_list))
    else:
        await matcher.finish(header + '昨日没有记录')


who_speak_most = on_command('排行', priority=2, aliases={'谁话多', '谁屁话最多', '排行', '排行榜'}, block=True)


@who_speak_most.handle()
async def _(bot: Bot, matcher: Matcher, event: GroupMessageEvent, args: Message = CommandArg()):
    try:
        gid, header = await _resolve_cross_group(bot, event, args)
    except Exception:
        await matcher.finish('无法查询该群，可能机器人不在该群中')
    dic_ = _load_counter(group_message_data_path / f"{gid}" / 'history.json')
    if not dic_:
        await matcher.finish(header + '没有任何人说话')
    top = sorted(dic_.items(), key=lambda x: x[1], reverse=True)
    top = (await member_in_group(bot, gid, top))
    if len(top) == 0:
        await matcher.finish(header + '没有任何人说话')
    top_list = []
    for i in range(min(len(top), 10)):
        nickname = await safe_get_nickname(bot, gid, int(top[i][0]))
        top_list.append(f"{i + 1}. {nickname}，发了{top[i][1]}条消息")
    await matcher.finish(header + '\n'.join(top_list))


get_speak_num = on_command('发言数', priority=2, aliases={'发言数', '发言', '发言量'}, block=True)


@get_speak_num.handle()
async def _(bot: Bot, matcher: Matcher, event: GroupMessageEvent, at_list: list = Depends(msg_at)):
    gid = event.group_id
    dic_ = _load_counter(group_message_data_path / f"{gid}" / 'history.json')
    if not at_list:
        await matcher.finish('请@要查询的人')
    if not dic_:
        await matcher.finish('没有任何人说话')
    if at_list:
        for qq in at_list:
            nickname = await safe_get_nickname(bot, gid, int(qq))
            qq = str(qq)
            if qq in dic_:
                await matcher.send(f"有记录以来{nickname}在本群发了{dic_[qq]}条消息")
            else:
                await matcher.send(f"{nickname}没有发消息")


get_speak_num_today = on_command('今日发言数', priority=2, aliases={'今日发言数', '今日发言', '今日发言量'}, block=True)


@get_speak_num_today.handle()
async def _(bot: Bot, matcher: Matcher, event: GroupMessageEvent, at_list: list = Depends(msg_at)):
    gid = event.group_id
    today = datetime.date.today().strftime('%Y-%m-%d')
    dic_ = _load_counter(group_message_data_path / f"{gid}" / f"{today}.json")
    if not at_list:
        await matcher.finish('请@要查询的人')
    if not dic_:
        await matcher.finish('没有任何人说话')
    if at_list:
        for qq in at_list:
            nickname = await safe_get_nickname(bot, gid, int(qq))
            qq = str(qq)
            if qq in dic_:
                await matcher.send(f"今天{nickname}发了{dic_[qq]}条消息")
            else:
                await matcher.send(f"今天{nickname}没有发消息")


async def member_in_group(bot: Bot, gid: int, top: list):
    """过滤已退群成员"""
    member_dict = await bot.get_group_member_list(group_id=gid)
    member_ids = {m['user_id'] for m in member_dict}
    return [j for j in top if int(j[0]) in member_ids]


async def safe_get_nickname(bot: Bot, gid: int, uid: int) -> str:
    """安全获取群成员昵称，退群成员返回QQ号"""
    try:
        info = await bot.get_group_member_info(group_id=gid, user_id=uid)
        return info.get('card') or info.get('nickname') or str(uid)
    except Exception:
        return str(uid)
