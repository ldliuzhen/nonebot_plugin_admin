...
async def init():
    ...
    if not group_alias_path.exists():
        default_alias = {
            "虫海一群": "925515105", "一群": "925515105",
            "虫海二群": "1043762671", "二群": "1043762671",
            "恐龙群": "533430073", "恐龙": "533430073", "侏罗纪": "533430073"
        }
        await mk('file', group_alias_path, 'w',
                 content=json.dumps(default_alias, ensure_ascii=False, indent=2))
    if not pm_bindings_path.exists():
        await mk('file', pm_bindings_path, 'w', content='{}')
    if not auto_reply_path.exists():
        await mk('file', auto_reply_path, 'w', content='{"global": {}}')
    if not deputy_perms_path.exists():
        await mk('file', deputy_perms_path, 'w', content=json.dumps({
            "default": {"ban": True, "kick": True, "black": True, "change": True,
                        "recall": True, "essence": True, "auto_reply": True}
...
async def mute_sb(bot: Bot, gid: int, lst: list, time: int = None, scope: list = None, max_time: int = None):
    """
    构造禁言
    :param gid: 群号
    :param time: 时间（s)
...
def resolve_group_alias(name: str) -> Optional[str]:
    """通过别名查找群号，返回群号字符串，未找到返回 None"""
    try:
        aliases = json_load(group_alias_path) or {}
    except Exception:
        try:
            with open(group_alias_path, mode='r', encoding='gbk') as f:
                aliases = json.load(f)
        except Exception:
            return None
...
_DEPUTY_OPS = ("ban", "kick", "black", "change", "recall", "essence", "auto_reply")


def get_deputy_perm(gid: str, operation: str) -> bool:
    data = json_load(deputy_perms_path)
    if data is None:
        return True
    group_cfg = data.get(gid, {})
    if operation in group_cfg:
        return bool(group_cfg[operation])
    default_cfg = data.get("default", {})
    return bool(default_cfg.get(operation, True))


def set_deputy_perm(gid: str, operation: str, enabled: bool) -> None:
    data = json_load(deputy_perms_path) or {
        "default": {op: True for op in _DEPUTY_OPS}
    }
    if gid not in data:
        data[gid] = {}
    data[gid][operation] = enabled
    json_upload(deputy_perms_path, data)


def get_all_deputy_perms(gid: str) -> dict:
    return {op: get_deputy_perm(gid, op) for op in _DEPUTY_OPS}
