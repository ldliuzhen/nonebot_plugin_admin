...
async def _(bot: Bot, matcher: Matcher, event: MessageEvent, state: T_State, args: Message = CommandArg()):
    if args:
        if any(i in str(args) for i in ["取消", "算了", "退出"]):
...
async def _(
    ...
    if any(i in str(b_args) for i in ["取消", "算了", "退出"]):
        await fi(matcher, "已取消广播")
        return
...
async def del_avoid_group(event: MessageEvent, args, matcher: Matcher):
    args = str(args).replace("-", "").strip()
    uid = str(event.user_id)
    groups = [g for g in str(args).split(" ") if g]
    try:
        history_group = broadcast_config[uid]
        deleted = []
        for g in groups:
        ...
        broadcast_config.update({uid: history_group})
        json_upload(broadcast_avoid_path, broadcast_config)
        r = '\n'.join(deleted) if deleted else '没有匹配的群'
...
async def broadcast(uid: str, args: Message, bot: Bot, matcher: Matcher):
    b_config = json_load(broadcast_avoid_path) or {}
    if uid not in b_config:
        b_config[uid] = []
        json_upload(broadcast_avoid_path, b_config)
